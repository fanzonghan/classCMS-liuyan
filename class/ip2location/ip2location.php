...
class ip2location
{
    ...
    private $lastip;
    private $totalip;
    private $dict_isp = array('联通','移动','铁通','电信','长城','聚友');
    private $dict_city_directly = array('北京','天津','重庆','上海');
    private $dict_province = array('北京','天津','重庆','上海','河北','山西','辽宁','吉林','黑龙江','江苏','浙江','安徽','福建','江西','山东','河南','湖北','湖南','广东','海南','四川','贵州','云南','陕西','甘肃','青海','台湾','内蒙古','广西','宁夏','新疆','西藏','香港','澳门');
    function __construct($filepath = null)
    {
        $filename =classDir(__CLASS__).'qqwry.dat';
	    if ($filepath) {
            $filename = $filepath;
        }
    ...
    function auth() {
        Return array('index'=>'IP查询');
    }
    function index() {
        $array['breadcrumb'][0]=array('title'=>'IP查询');
        if(isset($_GET['ip'])) {
            $array['ip']=$_GET['ip'];
            $array['info']=C('ip2location:get',$_GET['ip']);$array['info1']=C('ip2location:get',$_GET['ip']);
        }else {
            $array['ip']='';
        }
        V('template',$array);
